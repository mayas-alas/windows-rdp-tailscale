...
2. In Windows, open `C:\OEM` and double-click `01-Install-GNX.cmd`. It starts the frozen `Quetzalcoatl-0.1.3-setup-x64.exe` through the normal UAC flow. The `90-Repair-GNX.cmd` and `99-Uninstall-GNX.cmd` helpers reuse that exact frozen bundle.
...
5. Double-click `03-Collect-GNX-Evidence.cmd`. It writes allowlisted evidence to a writable Dockur host share: `Z:\gnx-evidence` when `Z:` exists, otherwise the current user's `Desktop\Shared\gnx-evidence` folder. If neither supported location is writable, it fails without writing local-only evidence. Service evidence is only the service whose name is exactly `Quetzalcoatl` (case-insensitive), with the expected `NT SERVICE\Quetzalcoatl` identity.
6. The workflow accepts only a structurally validated evidence envelope: the exact top-level keys, one exact service record, typed binary-hash records (including `gnx.exe`), and one unambiguous status branch. That branch is either a recursively schema-valid, allowlisted `gnx status --json` object with no error, or null status with the fixed `status-command-exited-nonzero-before-interactive-configuration` category. Artifact collection copies `gnx-evidence.json` only after that same validation; otherwise it stores only a fixed invalid-or-missing marker.
...
The noVNC session is explicitly limited to 60, 120, or 180 minutes. It fails when that bound expires without valid exported guest evidence, then is cleaned up. This compatibility rule remains distinct from G5 physical quorum evidence. Guest RDP is not published. No credentials, passwords, or auth keys are placed in the job summary. Evidence artifacts contain allowlisted host diagnostics plus either the validated allowlisted guest envelope or, when validation fails, the fixed invalid-or-missing marker; they are retained for one day.
